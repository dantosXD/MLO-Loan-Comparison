...
export const calculateDTIForProgram = (program: Program, loanData: LoanData): number => {
  if (loanData.grossMonthlyIncome <= 0) return 0;
  
  const totalPITI = calculateTotalPITI(program, loanData);
  const totalDebts = totalSelectedMonthlyDebts(loanData.debts);
  const totalMonthlyObligations = totalPITI + totalDebts;
  
  return (totalMonthlyObligations / loanData.grossMonthlyIncome) * 100;
};
